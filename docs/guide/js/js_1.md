...
```html  
<script type="text/js" src="文件路径"></script>  
```
## 输出语句  
...
JS中每条语句以分号(;)结尾如果不写分号，浏览器会自动添加，但是会消耗一些系统资源，  而且有些时候，浏览器会加错分号，所以在开发中分号必须写    可用 VScode 插件自动格式化
